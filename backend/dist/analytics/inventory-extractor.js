 ...
 */
export function extractMgoRating(productName) {
...
function inferProductCategory(productName) {
    if (/tincture/i.test(productName))
        return "Tincture";
    if (/blend|immunity|energy|recovery/i.test(productName))
        return "Bioactive Blend";
    if (/honey|mgo/i.test(productName))
        return "Honey";
    return "Other";
}
/**
 ...
 */
function buildProductNotes(productName) {
    const notes = [];
 ...
 */
export function extractInventoryData(rows, 

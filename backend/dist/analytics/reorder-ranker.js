 ...
 */
export function rankReorders(feedRows, minRiskLevel = "medium") {
    ...
    const filtered = recommendations.filter((r) => riskLevelPriority[r.riskLevel] >= minRiskScore);
 ...
 */
export function categorizeByConflict(recommendations) {
    const immediate = [];
    ...
    for (const rec of recommendations) {
        ...
        const hasConflict = detectRevenueVsTrendConflict(rec.revenueOpportunity, rec.momentumTrend, rec.daysOfCover);
        if (hasConflict) {
...
//# sourceMappingURL=reorder-ranker.js.map
 ...
 */
export async function runAnalystResponseNode(state, env) {
...
Challenges:
${review.auditor_challenges.map((ch) => `  ${ch.id} (${ch.type}/${ch.severity}): ${ch.question}
   Evidence: ${ch.evidence}
   Requested Action: ${ch.requestedAction}`).join('\n\n')}`)
        .join('\n\n---\n\n');
...
Be specific. Reference challenge IDs and provide evidence or clarification.`;
    ...
    try {
        ...
        const responsesResponse = JSON.parse(jsonStr);
        // Validate structure
        ...
        if (responsesResponse.responses.length !== 5) {
            throw new Error(`Expected 5 section responses from analyst, got ${responsesResponse.responses.length}`);
        }
        // Validate each response
        for (const resp of responsesResponse.responses) {
            if (!resp.section_id || !resp.analyst_response || !resp.resolution_type) {
                throw new Error(`Response for section ${resp.section_id || '(unknown)'} missing required field: section_id, analyst_response, or resolution_type`);
            }
        }
        return {
...
//# sourceMappingURL=analyst-response-node.js.map
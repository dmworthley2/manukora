 ...
 */
export async function runAuditorFinalizerNode(state, env) {
...
- Analyst and you fundamentally disagree on approach (both positions go to CEO)`;
    try {
        ...
        const decisionsResponse = JSON.parse(jsonStr);
        // Validate structure
        ...
        if (decisionsResponse.decisions.length !== 5) {
            throw new Error(`Expected 5 section decisions from auditor, got ${decisionsResponse.decisions.length}`);
        }
        // Validate each decision
        for (const decision of decisionsResponse.decisions) {
            if (decision.section_id === undefined || decision.is_approved === undefined) {
                throw new Error(`Decision for section ${decision.section_id || '(unknown)'} missing required field: section_id or is_approved`);
            }
            // If not approved, must have escalation_reason
            if (!decision.is_approved && !decision.escalation_reason) {
                throw new Error(`Section ${decision.section_id} is not approved but has no escalation_reason`);
            }
        }
        return {
...
//# sourceMappingURL=auditor-finalizer-node.js.map
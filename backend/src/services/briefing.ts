 ...
 */
function formatBriefingAsMarkdown(state: BriefingState): string {
 ...
 */
export async function saveBriefingToStorage(
 ...
 */
export async function updateReportRunWithBriefing(
  client: SupabaseClient,
  reportRunId: string,
  state: BriefingState,
...
export async function finalizeBriefing(
  ...
  const storagePath = await saveBriefingToStorage(
    client,
    state,
    reportRunId,
  );

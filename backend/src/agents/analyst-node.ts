...
import type { BriefingState, AnalystDraftedSection } from "./types.js";
import type { AnalystSection } from "../services/briefing-blackboard.js";
...
export async function runAnalystNode(
  ...
  const inventoryContext =
    state.inventoryReasoningFeed && state.inventoryReasoningFeed.length > 0
      ? `\n\n## Inventory Context (Reasoning-Based Decision Support)\n\nPre-calculated metrics for CFO-level capital allocation decisions:\n\n${state.inventoryReasoningFeed
          .map((row) => {
            ...
            const growthRate =
              m1Sales > 0 ? Math.round(((m4Sales - m1Sales) / m1Sales) * 100) : 0;
            const declineRate =
              m1Sales > 0 ? Math.round(((m1Sales - m4Sales) / m1Sales) * 100) : 0;
            const monthlyBurn = row.current_demand || 0;
            const daysUntilStockout =
              monthlyBurn > 0 ? Math.round((row.stock_on_hand / monthlyBurn) * 30) : 999;
            const isPhaseout =
              row.product_name?.toLowerCase().includes("propolis") ||
              row.sku?.toLowerCase().includes("propolis");
            const isPremium = row.sku?.toLowerCase().includes("1700");
            const isBioactive =
              row.product_name?.toLowerCase().includes("bioactive") ||
              row.sku?.toLowerCase().includes("immunity") ||
              row.sku?.toLowerCase().includes("energy") ||
              row.sku?.toLowerCase().includes("recovery");
            const onOrderNotYetArrived =
              row.units_on_order > 0 ? `, next shipment arrives in ${row.order_arrival_months} month(s)` : "";
            const leadTimeGap =
              row.units_on_order === 0 && row.order_arrival_months > 0
  ...
  if (state.auditorReviews && state.auditorReviews.length > 0) {
    ...
    const challengeSummary = state.auditorReviews
      ...
      .map(review =>
        `Section: ${review.section_id}\nChallenges:\n${review.auditor_challenges!.map(ch =>
          `  - ${ch.id} (${ch.type}): ${ch.question}`
        ).join('\n')}`
      )
      .join('\n\n');
...
Generate the 5-section JSON with analyst_draft and analyst_reasoning for each section, including certainty factors [CERTAINTY: HIGH/MEDIUM/LOW] as specified in the system prompt.`;
  ...
  try {
    ...
    const response: { sections: AnalystSection[] } = JSON.parse(jsonStr);

    // Validate we got all 5 sections
    if (!response.sections || !Array.isArray(response.sections) || response.sections.length !== 5) {
      throw new Error(
        `Expected 5 sections in response, got ${response.sections?.length || 0}`
      );
    }
    ...
    for (const section of response.sections) {
      if (!section.section_id || !section.title || !section.analyst_draft) {
        throw new Error(
          `Section ${section.section_id || '(unknown)'} missing required field: section_id, title, or analyst_draft`
        );
      }
    }

    return {
      analystDraft: response.sections as unknown as readonly AnalystDraftedSection[],

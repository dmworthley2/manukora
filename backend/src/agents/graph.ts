  ...
  try {
    ...
    while (state.iterationCount < 2) {
      ...
      if (client && state.blackboardId && state.auditorReviews) {
        const submitResult = await submitAuditorChallenges(client, state.blackboardId, Array.from(state.auditorReviews) as AuditorReview[]);
        if (!submitResult.success) {
          return {
            ...state,
      ...
      if (!allApproved && state.iterationCount < 1) {
        ...
        if (client && state.blackboardId && state.auditorFinalDecisions) {
          const finalizeResult = await finalizeAuditorDecisions(
            client,
            state.blackboardId,
            Array.from(state.auditorFinalDecisions) as AuditorFinalDecision[],
          );
          if (!finalizeResult.success) {
            return {
              ...state,

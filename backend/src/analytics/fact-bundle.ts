 ...
 */
export function buildFactBundle(rows: readonly CommercialDataRow[]): FactBundle {
  if (rows.length === 0) {
...
function generateRecommendations(
  valueAtRisk: readonly SkuValueAtRisk[],
  coverRisks: readonly SkuCoverRisk[],
  trends: readonly SkuTrend[],
  metrics: readonly SkuMonthMetrics[],
  ...
  for (const trend of trends) {
    if (recommendations.length >= 3) break;
    if (seen.has(trend.sku)) continue;
    if (!trend.isDecline) continue;
    ...
    recommendations.push({
      ...
      suggestedAction:
        "De-prioritize reorder; monitor demand before committing. Consider promotional clearance.",
      metrics: {
        valueAtRisk: Math.round(val.valueAtRisk),
        daysOfCover: risk.daysOfCover,
...
function identifyProactiveRisks(
  coverRisks: readonly SkuCoverRisk[],
  trends: readonly SkuTrend[],
): ProactiveRisk[] {
  ...
  for (const coverRisk of coverRisks) {
    ...
    if (coverRisk.daysOfCover > 60 && trend?.isDecline) {
      risks.push({
        ...
        suggestedMitigation:
          "Consider promotional clearance or slower reorder cadence to avoid obsolescence.",
      });
    }
    ...
    if (coverRisk.daysOfCover > 90) {
      risks.push({
        ...
        suggestedMitigation:
          "Analyze seasonal patterns. Defer routine reorder until demand pattern clarifies.",
      });
    }
  }
